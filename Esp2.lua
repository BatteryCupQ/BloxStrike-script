-- ESP+ с современной панелью, RGB, Distance, AIM (БЕЗ SKYBOX)
...
local function createSlider(parent, text, getter, setter, min, max, format)
    ...
    uis.InputChanged:Connect(function(input)
        if dragging and input.UserInputType == Enum.UserInputType.MouseMovement then
            ...
            local percent = math_clamp(pos / sliderBg.AbsoluteSize.X, 0, 1)
...
local function createRGBControl(parent, text, colorRef)
    ...
    local function createChannel(ch, x, default)
        ...
        box.FocusLost:Connect(function()
            local val = math_clamp(tonumber(box.Text) or 0, 0, 255) / 255
...
local function createModernPanel()
    ...
    aimTab.CanvasSize = UDim2.new(0,0,0,200)
