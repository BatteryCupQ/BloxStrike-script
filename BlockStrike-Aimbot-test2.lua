-- Антидетект ESP: обводка, HP, скелет + расширение хитбокса + ПОСТОЯННЫЙ АИМ (always-on) + ВКЛАДКИ + ДИСТАНЦИЯ + ТРАССЕРЫ + ОРУЖИЕ
...
-- ========== НАСТРОЙКИ АИМА (ПОСТОЯННЫЙ) ==========
local aimSettings = {
    enabled = false,           -- вкл/выкл аим (переключается в меню)
    fov = 150,                  -- радиус действия (пиксели)
    smoothness = 0.5,           -- плавность (0.1-1)
    targetPart = "Head",        -- часть тела для наведения
    method = "mouse",           -- "mouse" (mousemoverel) или "camera" (CFrame)
...
-- Поиск ближайшей цели в FOV (без учёта стен)
...
local function aimAt(part)
    ...
    elseif aimSettings.method == "camera" then
        ...
        if myHead then
            local direction = (part.Position - myHead.Position).unit
            camera.CFrame = CFrame.lookAt(myHead.Position, myHead.Position + direction)
        end
    end
end

-- Генератор случайных строк (для скрытности)
...
local function createMenu()
    ...
    fovToggleBtn.Name = randomString(8)
    fovToggleBtn.Size = UDim2.new(0,220,0,30)
    fovToggleBtn.Position = UDim2.new(0.5,-110,0,yPosAim)
    fovToggleBtn.BackgroundColor3 = Color3.fromRGB(0,255,0)
    fovToggleBtn.Text = "FOV: ВКЛ"
    ...
    fovToggleBtn.Parent = aimContainer
    yPosAim = yPosAim + 35

    local btnCornerFov = Instance.new("UICorner")
    btnCornerFov.Name = randomString(6)
    btnCornerFov.CornerRadius = UDim.new(0,8)
    btnCornerFov.Parent = fovToggleBtn

    fovToggleBtn.MouseButton1Click:Connect(function()
        aimSettings.showFov = not aimSettings.showFov
        fovToggleBtn.BackgroundColor3 = aimSettings.showFov and Color3.fromRGB(0,255,0) or Color3.fromRGB(255,0,0)
        fovToggleBtn.Text = aimSettings.showFov and "FOV: ВКЛ" or "FOV: ВЫКЛ"
    end)
...
runService.Heartbeat:Connect(function()
    ...
    if aimSettings.showFov and aimSettings.enabled then
    ...
    -- Если аим включён, постоянно наводимся на ближайшую цель (даже за стеной)
    if aimSettings.enabled then
