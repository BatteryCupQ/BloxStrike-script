-- Антидетект ESP: обводка, HP, скелет + расширение хитбокса + ПРОСТОЙ АИМ + ВКЛАДКИ + ДИСТАНЦИЯ + ПЛАВАЮЩЕЕ ОКНО
...
-- ========== НОВЫЙ ПРОСТОЙ АИМ (БЕЗ КОНФЛИКТОВ) ==========
local SimpleAim = {
    enabled = false,
    targetPart = "Head"
}

-- Функция проверки тиммейта (используется и в ESP, и в аиме)
...
-- Функция поиска ближайшего игрока для аима
local function getClosestPlayer_Simple()
    local closest = nil
...
local function createMenu()
    ...
    -- === НАПОЛНЕНИЕ ВКЛАДКИ AIM (кнопка для нового аима) ===
    local yPosAim = 5

    local aimSimpleBtn = Instance.new("TextButton")
    aimSimpleBtn.Name = randomString(8)
    aimSimpleBtn.Size = UDim2.new(0,220,0,30)
    aimSimpleBtn.Position = UDim2.new(0.5,-110,0,yPosAim)
    ...
    aimSimpleBtn.BorderSizePixel = 0
    aimSimpleBtn.Parent = aimContainer

    local btnCornerAimSimple = Instance.new("UICorner")
    btnCornerAimSimple.Name = randomString(6)
    btnCornerAimSimple.CornerRadius = UDim.new(0,8)
    btnCornerAimSimple.Parent = aimSimpleBtn

    aimSimpleBtn.MouseButton1Click:Connect(function()
        SimpleAim.enabled = not SimpleAim.enabled
        if SimpleAim.enabled then
            aimSimpleBtn.BackgroundColor3 = Color3.fromRGB(0,255,0)
            aimSimpleBtn.Text = "SIMPLE AIM: ON"
        else
            aimSimpleBtn.BackgroundColor3 = Color3.fromRGB(255,0,0)
            aimSimpleBtn.Text = "SIMPLE AIM: OFF"
        end
...
-- ========== ПЛАВАЮЩЕЕ ОКНО ДЛЯ ПРОСТОГО АИМА ==========
...
runService.Heartbeat:Connect(function()
    if SimpleAim.enabled and player.Character and player.Character:FindFirstChild("HumanoidRootPart") then
        local target = getClosestPlayer_Simple()
        if target then
